...
def plot_client_epsilon_distribution(
    ...
    for key, value in run.data.metrics.items():
        if "client_epsilon" in key and value > 0:
            parts = key.split("_")
            round_num = int(parts[1])
            client_id = int(parts[3])
            if round_num == 1:
                client_epsilons[client_id] = value
    ...
    epsilons = [client_epsilons[cid] for cid in client_ids]
    ...
    ax2.hist(epsilons, bins=min(10, len(epsilons)), color="steelblue", edgecolor="black", linewidth=0.5)
    ax2.set_xlabel("Epsilon (ε)")
...
def plot_cumulative_privacy_budget(
    ...
    for key, value in run.data.metrics.items():
        if "cumulative_epsilon" in key and value > 0:
            parts = key.split("_")
            round_num = int(parts[1])
            client_id = int(parts[3])
            if client_id not in client_cumulative:
                client_cumulative[client_id] = []
            client_cumulative[client_id].append((round_num, value))
    ...
    colors = plt.cm.tab10(np.linspace(0, 1, min(10, len(client_cumulative))))
    for i, (client_id, data) in enumerate(sorted(client_cumulative.items())):
        ...
        ax.plot(rounds_list, epsilons_list, marker="o", markersize=2, label=f"Client {client_id}", color=colors[i % len(colors)])
